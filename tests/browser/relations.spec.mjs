...
test('opening and revisiting preload profiles without drawing until requested', async ({ page }) => {
    await setup(page);
    ...
    await expect(page.locator('[data-node]')).toHaveCount(0);
    expect(await page.evaluate(() => globalThis.graphWorkerStarts)).toBe(2);
...
test('first-hop labels are visible until a path is selected, SVG fills its stage and colors match direction', async ({ page }) => {
    ...
    await page.locator('.fcm-graph-depth input').fill('101');
    await page.locator('.fcm-graph-depth input').press('Enter');
    await expect(page.locator('.fcm-graph-depth input')).toHaveValue('2');
...
test('1000 people with 3999 relationships remain interactive and render cached names', { tag: '@performance' }, async ({ page }) => {
 ...
 test('graph preloads without drawing, limits depth and toggles path names', async ({ page }) => {
    ...
    await expect(page.locator('[data-node="1"] text')).toBeVisible();
    await expect(page.locator('[data-node="2"] text')).toBeVisible();
    await expect(page.locator('[data-node="3"] text')).toBeHidden();
    await page.locator('.fcm-graph-stage svg').click({button:'right',position:{x:10,y:10}});
    ...
    await depth.fill('100'); await depth.press('Enter');
    await page.getByRole('button',{name:'關係層數 + 1',exact:true}).click();
...
test('zoom percentage tracks buttons, wheel and fit relative to the fitted view', async ({ page }) => {
    await setup(page);
    await page.evaluate(() => globalThis.graphTest.render());
    const percent = page.locator('.fcm-graph-zoom-percent');
    ...
    await expect(percent).toHaveText('100%');
    await page.locator('.fcm-graph-stage svg').dispatchEvent('wheel', {deltaY:-100});
    await expect(percent).not.toHaveText('100%');
    for (let i = 0; i < 15; i++) await page.getByRole('button', {name:'縮小',exact:true}).click();
    await expect(percent).toHaveText('10%');
    await page.getByRole('button', {name:'置中／適合視窗',exact:true}).click();
    await expect(percent).toHaveText('100%');
});
