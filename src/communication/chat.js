...
function avatarHtml(memberNumber, size = 34) {
    const url = avatarUrl(memberNumber);
    const status = isOnline(memberNumber) ? (sharedProfile(memberNumber).status || 'online') : 'offline';
    return `<span class="fcm-chat-avatar ${cfg.chatAvatarShape === 'round' ? 'round' : 'square'}" style="width:${size}px;height:${size}px">
...
function bindEvents() {
    ...
    root.querySelector('[data-search]')?.addEventListener('input', event => { search = event.target.value; renderChat(); root.querySelector('[data-search]')?.focus(); });
    ...
    root.querySelector('[data-assign-group]')?.addEventListener('change', event => {
        if (!selectedMember || !event.target.value) return;
        cfg.chatMemberGroups ||= {}; cfg.chatMemberGroups[selectedMember] ||= [];
        if (!cfg.chatMemberGroups[selectedMember].includes(event.target.value)) cfg.chatMemberGroups[selectedMember].push(event.target.value);
        saveCfg(); event.target.value = '';
    });
    ...
    root.querySelector('[data-custom-sound]')?.addEventListener('change', async event => {
        const file = event.target.files?.[0]; if (!file || !await AudioStore.save(file)) return;
        if (customAudioUrl) URL.revokeObjectURL(customAudioUrl); customAudioUrl = URL.createObjectURL(file); cfg.notificationSound = 'custom'; saveCfg(); renderChat();
    });
    ...
    root.querySelector('[data-chat-theme-mode]')?.addEventListener('change', event => { cfg.chatThemeMode = event.target.value; saveCfg(); renderChat(); refreshChatSettings(); });
...
function ensureBalloon(force = false) {
    ...
    if (!balloon) {
        ...
        balloon.addEventListener('click', () => { if (!balloon.dataset.dragMoved) openChat(latestIncoming?.memberNumber); });
...
function showIncomingBalloon(message) {
    if (cfg.individualBalloons) {
        ...
        if (!balloon) {
            ...
            balloon.addEventListener('click', () => { if (!balloon.dataset.dragMoved) openChat(message.memberNumber); });
            document.body.appendChild(balloon);
...
.fcm-chat-avatar{position:relative;display:inline-flex;flex:none;align-items:center;justify-content:center;border-radius:7px;overflow:visible;background:var(--surface-alt);color:var(--ac);border:1px solid var(--border);font-size:11px;font-weight:700}.fcm-chat-avatar img{width:100%;height:100%;border-radius:inherit;object-fit:cover;pointer-events:none}.fcm-chat-avatar i{position:absolute;right:-2px;bottom:-2px;width:9px;height:9px;border:2px solid var(--s);border-radius:50%;background:#58c878}.fcm-chat-avatar i.busy,.fcm-status-dot.busy,.fcm-chat-status-menu i.busy{background:#e85d68}.fcm-chat-avatar i.afk,.fcm-status-dot.afk,.fcm-chat-status-menu i.afk{background:#e9bd4b}.fcm-chat-avatar i.offline,.fcm-status-dot.offline,.fcm-chat-status-menu i.offline{background:#777}
