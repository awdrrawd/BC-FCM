function createChatMemberSelection({ getRoot, getMemberNumber, setMemberNumber, resetSelection, clearReply, closeContactCard, setStackedDetail, chatStore, setMessageIndex, loadConversation, refreshBadges, getLayout, refreshConversation, unreadCount }) {
    ...
    async function select(memberNumber) {
        ...
        refreshBadges();
        const root = getRoot();
        root?.querySelectorAll('[data-member]').forEach(row => row.classList.toggle('selected', Number(row.dataset.member) === target));
