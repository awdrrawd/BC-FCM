...
import { ADD_FRIEND_ICON, CAMERA_ICON, DOWNLOAD_ICON, FOLDER_ICON, GROUP_ICON, INVITE_ICON, SEARCH_ICON, SUMMON_ICON, TRASH_ICON, EXIT_ICON } from '../../../ui/icons.js';
...
function conversationHtml(model) {
    ...
    <div class="fcm-chat-compose" ${model.multiSelect ? 'hidden' : ''}>
        ...
        <button type="button" data-upload-image hidden title="${TH('chatUploadImage')}" aria-label="${TH('chatUploadImage')}">${CAMERA_ICON}</button><div class="fcm-chat-input-wrap">${model.replyTarget ? `<div class="fcm-chat-reply-indicator"><span>${TH('chatReply')}: ${esc(model.replyTarget.preview)}</span><button data-cancel-reply title="${TH('chatCancel')}">×</button></div>` : ''}<div class="fcm-chat-profile-suggest" data-profile-suggest hidden></div><textarea data-input rows="2" placeholder="${esc(model.inputPlaceholder)}"></textarea></div>
