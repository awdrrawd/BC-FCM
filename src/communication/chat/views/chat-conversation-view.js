...
import { ADD_FRIEND_ICON, CAMERA_ICON, IMAGE_ICON, DOWNLOAD_ICON, FOLDER_ICON, GROUP_ICON, INVITE_ICON, SEARCH_ICON, SUMMON_ICON, TRASH_ICON, EXIT_ICON } from '../../../ui/icons.js';
...
function contactCardHtml({ memberNumber, avatarHtml, displayName, biography, hasProfile, isFriend }) {
    return `<section class="fcm-chat-contact-card">${avatarHtml(memberNumber, 100, 'card')}<div><b>${esc(displayName)} (${memberNumber})</b><small>${esc(biography || T('chatNoBiography'))}</small><span class="fcm-chat-card-actions"><button data-card-refresh title="${TH('chatProfileSnapshot')}" aria-label="${TH('chatProfileSnapshot')}">${CAMERA_ICON}</button>${hasProfile ? `<button class="fcm-chat-card-search" data-card-profile title="${TH('btnViewProfile')}" aria-label="${TH('btnViewProfile')}">${SEARCH_ICON}</button>` : ''}${isFriend ? '' : `<button data-card-add-friend title="${TH('addFriend')}">${ADD_FRIEND_ICON}${TH('addFriend')}</button>`}</span></div></section>`;
}
...
function conversationHtml(model) {
    ...
    <div class="fcm-chat-actions" ${model.multiSelect ? 'hidden' : ''}><button class="fcm-chat-icon-action" data-invite ${model.canInvite ? '' : 'disabled'} title="${TH('chatInviteRoom')}" aria-label="${TH('chatInviteRoom')}">${INVITE_ICON}</button><button type="button" class="fcm-chat-icon-action" data-upload-image hidden title="${TH('chatUploadImage')}" aria-label="${TH('chatUploadImage')}">${IMAGE_ICON}</button><span></span><div class="fcm-chat-tools"><div class="fcm-chat-tools-menu"><button class="fcm-chat-icon-action" data-export="html" title="${TH('chatExportHtml')}">${DOWNLOAD_ICON}<span>${TH('chatExportHtml')}</span></button><button class="fcm-chat-icon-action" data-export="json" title="${TH('chatExportJson')}">${DOWNLOAD_ICON}<span>${TH('chatExportJson')}</span></button><button class="fcm-chat-icon-action" data-delete title="${TH('chatDeleteAll')}">${TRASH_ICON}<span>${TH('chatDeleteAll')}</span></button></div><button class="fcm-chat-icon-action" data-toggle-tools title="${TH('chatMessageTools')}">${FOLDER_ICON}</button></div></div>
    <div class="fcm-chat-compose" ${model.multiSelect ? 'hidden' : ''}>
        <div class="fcm-chat-compose-notice" data-bcx-compose-notice hidden></div>
        <div class="fcm-chat-channels ${model.online ? '' : 'offline'}"><button class="${model.available === 'whisper' ? 'active' : ''}" data-channel="whisper" data-placeholder="${TH('chatWhisperInputPlaceholder')}" ${!model.canWhisper ? 'disabled' : ''}>${TH('btnWhisper')}</button><button class="${model.available === 'beep' ? 'active' : ''}" data-channel="beep" data-placeholder="${TH('chatPrivateInputPlaceholder')}" ${!model.online ? 'disabled' : ''}>${TH('btnBeep')}</button></div>
