...
function profileHtml({ Player, cfg, T, esc, avatarHtml, editIcon }) {
    ...
    return `<div class="fcm-chat-profile">
        ...
        <label class="fcm-chat-profile-field"><span class="fcm-profile-url-label">${T('chatProfileUrlAvatar')} <button type="button" class="fcm-profile-help" title="${esc(T('chatAvatarUrlHelp'))}">?</button></span><input data-profile-avatar-url value="${esc(cfg.chatAvatarUrl || cfg.avatarUrl || '')}" placeholder="https://…"></label>
